...
class Product(models.Model):
    ...
    def cases_unshipped(self):
        return Shipment.objects.filter(transaction__product=self, status__lt=Shipment.Status.SHIPPED).aggregate(cases_unshipped=Sum('transaction__cases'))['cases_unshipped'] or 0
    ...
    def cases_remaining_on_date(self, on_date):
        history = self.get_history(on_date)
        if not history:
            return self.cases_inventory
