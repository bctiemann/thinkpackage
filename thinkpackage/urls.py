...
#    url(r'^location_form/(?P<location_id>\d+)/$', ims_views.mgmt_location_form, name='mgmt-location-form'),
    ...
    url(r'^client/(?P<pk>\d+)/$', ims_views.ClientUpdate.as_view(), name='mgmt-client-update'),
    ...
    url(r'^(?P<pk>\d+)/$', ims_views.mgmt_redirect, name='mgmt-redirect'),
#    url(r'^(?P<client_id>\d+)/profile/$', ims_views.mgmt_profile, name='mgmt-profile'),
    url(r'^(?P<pk>\d+)/profile/$', ims_views.ClientUpdate.as_view(), name='mgmt-profile'),
    url(r'^(?P<pk>\d+)/profile/location=(?P<location_id>\d+)/$', ims_views.mgmt_profile, name='mgmt-profile'),
    url(r'^(?P<pk>\d+)/profile/contact=(?P<custcontact_id>\d+)/$', ims_views.mgmt_profile, name='mgmt-profile'),
    url(r'^(?P<pk>\d+)/inventory/(?P<product_id>\d+)?/?$', ims_views.mgmt_inventory, name='mgmt-inventory'),
    url(r'^(?P<pk>\d+)/shipments/(?P<shipment_id>\d+)?/?$', ims_views.mgmt_shipments, name='mgmt-shipments'),
]
